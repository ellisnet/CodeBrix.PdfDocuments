...
public class CreateSimplePdf
{
    ...
    [Fact]
    public void CreateTestPdfWithImageViaCodeBrixImaging()
    {
        using var stream = new MemoryStream();
        var document = new PdfDocument();
        ...
        // Load image for CodeBrix.Imaging and apply a simple mutation:
        var image = Image.Load<Rgb24>(PathHelper.GetInstance().GetAssetPath("lenna.png"), out var format);
        image.Mutate(ctx => ctx.Grayscale());

        // create XImage from that same CodeBrix.Imaging image:
