...
OVERVIEW
--------
CodeBrix.PdfDocuments and CodeBrix.PdfDocCreate are TWO companion .NET libraries
...
They are forks of the popular PdfSharpCore (v1.3.67) and MigraDocCore (v1.3.67)
libraries, licensed under MIT License.
...
However, ALL namespaces use "CodeBrix.PdfDocuments" and "CodeBrix.PdfDocCreate"
instead of "PdfSharp"/"MigraDoc". Do NOT mix the libraries.
...
INSTALLATION
------------
There are TWO NuGet packages. Install one or both depending on your needs:
...
WHEN TO USE WHICH:
  ...
  - Use both together when you need the document model AND custom drawing.
...
Do NOT attempt to use CodeBrix.PdfDocuments for the following:

  - Extracting text content from PDFs (no OCR, no text extraction API)
  - Converting PDFs to images (no rasterization)
  - Filling PDF forms (no form field support)
  - Digital signatures
...
This library IS for: creating new PDF documents, drawing text/images/graphics
on PDF pages, merging multiple PDFs, adding security/encryption, creating
structured documents with styles/tables/headers using the document model,
and adding bookmarks/outlines.
...
1. DO NOT confuse the two NuGet package names with their namespaces.
